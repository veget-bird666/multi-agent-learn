...
2. **画像未构建**（profile=空），且对话信息足够抽取特征 → profile_agent
3. **画像已存在，但新对话中出现与画像矛盾或明显补充的信息**（例如学生说"其实我更喜欢看公式推导"而现有画像标注为"举例型"）→ profile_agent，并填写 profile_update_hint 说明要更新的维度
4. **学生提出要学习某个知识点或需要学习路线** → rewrite_node（重写需求后自动触发路径规划）
5. **学生提出要学习某个知识点或需要学习路线，且学习路线已存在** → chat_agent（直接回复学习路线，无需重新规划）
6. **以上都不满足，只需正常回复** → chat_agent
7. **本轮交互已结束，无需后续操作** → FINISH
...
def supervisor_agent(state: LearningState):
    ...
    if result.next_agent == "rewrite_node" and learning_path:
        print(f"[Supervisor] 🛡️  路径已规划，拦截 rewrite_node → chat_agent")
