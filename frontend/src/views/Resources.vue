<template>
  <div class="max-w-5xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-900">我的学习资源</h2>
        <p class="text-sm text-gray-400 mt-1">所有对话中生成的资源集中管理</p>
      </div>
      <button
        @click="refresh"
        :disabled="loading"
    ...
    <!-- 空态 -->
    <div v-else-if="groups.length === 0" class="bg-white rounded-2xl border border-gray-100 p-16 text-center">
      <div class="text-5xl mb-4">📦</div>
      <h3 class="text-lg font-semibold text-gray-700 mb-2">暂无学习资源</h3>
      <p class="text-sm text-gray-400 mb-6">开始对话学习并生成资源后，它们会出现在这里</p>
      <router-link
        to="/chat"
    ...
    <div v-else class="space-y-8">
      <div v-for="(group, gi) in groups" :key="gi">
        <div class="flex items-center gap-2 mb-3">
          <h3 class="font-semibold text-gray-800">{{ group.knowledge_point }}</h3>
          <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{{ group.items.length }} 项</span>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            ...
            <button
              ...
              class="absolute top-2 right-2 w-7 h-7 bg-white/80 backdrop-blur-sm rounded-full border border-gray-200 text-gray-400 hover:text-red-500 hover:border-red-200 opacity-0 group-hover/card:opacity-100 transition-all flex items-center justify-center text-xs shadow-sm"
              title="删除此资源"
            >
              ✕
            </button>
    ...
    <div
      v-if="toast"
      class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm shadow-lg z-50 animate-fade-in"
    >
      {{ toast }}
    </div>
  </div>
